...
def main(args, config):
    ...
    optimizer = torch.optim.AdamW(params=model_without_ddp.parameters(), lr=config['init_lr'], weight_decay=config['weight_decay']) 
    ...
    for epoch in range(0, config['max_epoch']):   
        ...
        if epoch%2==0:
            ...
            if utils.is_main_process():       
                ...
                # Log losses and accuracy to wandb
                wandb.log({
                    "train_noun_loss": log_stats['loss_noun'], 
                    "train_verb_loss": log_stats['loss_verb'],    
                    "test_action_acc":   log_stats['t_acc'],
                    "test_noun_acc": log_stats['n_acc'],
                    "test_verb_acc": log_stats['v_acc'],
                })
